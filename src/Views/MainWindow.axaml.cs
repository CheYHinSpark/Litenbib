...
namespace Litenbib.Views
{
    public partial class MainWindow : StyledWindow
    {
        ...
        protected async override void OnInitialized()
        {
            ...
            {
                _viewModel = mwvm;
                var config = await _viewModel.LoadLocalConfig();
                ApplyWindowState(config);
            }
        }
        ...
        protected override async Task<bool> OnCloseButtonClicked()
        {
            ...
            if (_viewModel.NeedSave)
            {
                ...
                {
                    if (!await _viewModel.SaveAllFilesAsync())
                    {
                        return false;
                    }
                }
        ...
        private void ApplyWindowState(LocalConfig? config)
        {
            ...
            if (config.WindowWidth is > 0)
            {
                Width = config.WindowWidth.Value;
            }
            if (config.WindowHeight is > 0)
            {
                Height = config.WindowHeight.Value;
            }
            if (config.WindowPositionX.HasValue && config.WindowPositionY.HasValue)
            {
                Position = new PixelPoint(
                    (int)config.WindowPositionX.Value,
                    (int)config.WindowPositionY.Value);
            }
            if (config.WindowState.HasValue)
            {
                WindowState = config.WindowState.Value;
            }
        }
